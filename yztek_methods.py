...
def setUp():
    ...
    if driver.current_url == locators.moodle_url and driver.title == 'Zahra\'s Software Quality Assurance Testing':
        print(f'We\'re at Moodle homepage -- {driver.current_url}')

        print(f'We\'re seeing title message -- Zahra\'s Software Quality Assurance Testing')
    else:
...
def create_new_user():
    ...
    print('______________******________******_______******_____******______******_________')

def user_info():
    locators.new_username = f'{locators.first_name[:1]}{locators.last_name}{locators.fake.pyint(111,999)}'.lower()
    locators.new_password = locators.fake.password()[:12]
    locators.full_name = f'{locators.first_name} {locators.last_name}'
    locators.email = f'{locators.new_username}@{locators.fake.free_email_domain()}'
    locators.city = locators.fake.city()
...
def search_user():
    ...
    if driver.current_url == locators.moodle_users_main_page:
        #assert driver.find_element(By.XPATH, "//h1[text() ='Zahra\'s Software Quality Assurance Testing']").is_displayed()
        driver.find_element(By.LINK_TEXT, 'Show more...').click()
        sleep(3)
        if driver.find_element(By.LINK_TEXT, 'Email address') or driver.find_element(By.ID, 'email'):
            sleep(3)
            ...
            sleep(3)
            if driver.find_element(By.XPATH, f'//label[contains(., "{locators.email}")]'):
